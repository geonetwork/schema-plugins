...
<xsl:stylesheet version="2.0"
	...
	<!-- 
		This xslt transforms netcdf metadata to ISO19139mcp metadata that can be used by the thredds harvester to generate iso metadata from thredds catalogs.
		...
		<records>
			<xsl:for-each select="nc:netcdf[1]">
				...
				<record uuid="{$uuid}">
					...
					<fragment id="thredds.data.parameters" uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_data_parameters')}">
						<mcp:dataParameter>
							<mcp:DP_DataParameter>
								<xsl:apply-templates mode="ncml-variables" select="nc:variable"/>
							</mcp:DP_DataParameter>
						</mcp:dataParameter>
					</fragment>
					...
					<fragment id="thredds.revision.date" uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_metadata_revision_date')}">
						<mcp:revisionDate>
