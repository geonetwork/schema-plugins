		...
		<xsl:for-each select="gmd:identificationInfo/mcp:MD_DataIdentification|gmd:identificationInfo/srv:SV_ServiceIdentification">
			...
			<xsl:for-each select="gmd:citation/gmd:CI_Citation">
				...
				<xsl:for-each select="gmd:title/gco:CharacterString">
					...
					<Field name="title" string="{string(.)}" store="true" index="true" token="false"/>
				</xsl:for-each>
			...
			<xsl:for-each select="mcp:dataParameters/mcp:DP_DataParameters/mcp:dataParameter">
				<xsl:for-each select="mcp:DP_DataParameter/mcp:parameterName">
					<Field name="dataparam" string="{mcp:DP_ParameterName/mcp:name/gco:CharacterString}" store="true" index="true" token="true"/>
				</xsl:for-each>
			</xsl:for-each>
			...
			<xsl:for-each select="*/mcp:EX_Extent">
				...
				<xsl:for-each select="gmd:geographicElement/gmd:EX_GeographicDescription/gmd:geographicIdentifier/gmd:RS_Identifier">
					<xsl:if test="gmd:authority/gmd:CI_Citation/gmd:title/gco:CharacterString='c-squares'">
						<xsl:for-each select="tokenize(gmd:code/gco:CharacterString,'\|')">
							<Field name="csquare" string="{string(.)}" store="false" index="true" token="false"/>
						</xsl:for-each>
					</xsl:if>
				</xsl:for-each>
				...
				<xsl:for-each select="mcp:taxonomicElement/*/mcp:taxonConcepts/app:documents/TaxonConcept|mcp:taxonomicElement/*/mcp:taxonConcepts/app:documents/TaxonName">
					<Field name="taxon:genus"		string="{string(Genus)}" store="true" index="true" token="false"/>
					<Field name="taxon:epithet"	string="{string(SpecificEpithet)}" store="true" index="true" token="false"/>
					<Field name="taxon:code"		string="{string(NomenclaturalCode)}" store="true" index="true" token="false"/>
					<Field name="taxon:pub"			string="{string(PublicationRef)}" store="true" index="true" token="false"/>

					<!-- index both complete name and lsid of this species -->
					<Field name="taxon:name"		string="{string(NameComplete)}" store="true" index="true" token="false"/>
					<Field name="taxon:name"		string="{string(@id)}" store="true" index="true" token="false"/>
					...
					<xsl:for-each select="AcceptedFor/AcceptedForNameRef">
						...
						<xsl:if test="$complete!=''">
							<Field name="taxon:name"		string="{$complete}" store="true" index="true" token="false"/>
						</xsl:if>
						<xsl:variable name="ibisId" select="normalize-space(@ibis:objectidRef)"/>
						<xsl:if test="$ibisId!=''">
							<Field name="taxon:name"		string="{concat('urn:lsid:biodiversity.org.au:apni.taxon:',$ibisId)}" store="true" index="true" token="false"/>
						</xsl:if>
					</xsl:for-each>
	...
	<xsl:template match="*" mode="latLon">
		...
		<xsl:choose>
			...
			<xsl:when test="$eastBL &lt; $westBL">
				<Field name="crossesMeridian" string="true" store="true" index="true" token="false"/>
				<!-- Split extents crossing anti-meridian into two bounding boxes for searching purposes -->
				<!-- Eastern bounding box -->
				<Field name="westBL"   string="{$westBL + 360}" store="true" index="true" token="false"/>
				<Field name="eastBL"   string="{180 + 360}" store="true" index="true" token="false"/>
				<Field name="northBL"  string="{$northBL + 360}" store="true" index="true" token="false"/>
				<Field name="southBL"  string="{$southBL + 360}" store="true" index="true" token="false"/>
				<!-- Western bounding box -->
				<Field name="westBL2"  string="{-180 + 360}" store="true" index="true" token="false"/>
				<Field name="eastBL2"  string="{$eastBL + 360}" store="true" index="true" token="false"/>
				<Field name="northBL2" string="{$northBL + 360}" store="true" index="true" token="false"/>
				<Field name="southBL2" string="{$southBL + 360}" store="true" index="true" token="false"/>
			</xsl:when>
			<xsl:otherwise>		
				<Field name="crossesMeridian" string="false" store="true" index="true" token="false"/>
				<Field name="westBL"  string="{$westBL + 360}" store="true" index="true" token="false"/>
				<Field name="eastBL"  string="{$eastBL + 360}" store="true" index="true" token="false"/>
				<Field name="northBL" string="{$northBL + 360}" store="true" index="true" token="false"/>
				<Field name="southBL" string="{$southBL + 360}" store="true" index="true" token="false"/>
			</xsl:otherwise>
		</xsl:choose>	
