...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  exclude-result-prefixes="gmd gco gml gts srv xlink exslt geonet napm">

  <!-- Simple views is ISO19139 -->
  <xsl:template name="metadata-iso19139.napmview-simple">
    <xsl:call-template name="metadata-iso19139view-simple"/>
  </xsl:template>

  <xsl:template name="view-with-header-iso19139.napm">
    <xsl:param name="tabs"/>
  ...
  <xsl:template name="metadata-iso19139.napm" match="metadata-iso19139.napm">
    ...
    <xsl:variable name="profileElements">
      <xsl:apply-templates mode="iso19139.napm" select=".">
  ...
  <xsl:template mode="iso19139.napm" match="gmd:MD_Metadata|*[@gco:isoType='gmd:MD_Metadata']"
    priority="3">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:param name="embedded"/>
