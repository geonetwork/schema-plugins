...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="iso19139.mcp-1.4schema" select="/root/gui/schemas/iso19139.mcp-1.4"/>
  <xsl:variable name="iso19139.mcp-1.4labels" select="$iso19139.mcp-1.4schema/labels"/>
  <xsl:variable name="iso19139.mcp-1.4codelists" select="$iso19139.mcp-1.4schema/codelists"/>
  <xsl:variable name="iso19139.mcp-1.4strings" select="$iso19139.mcp-1.4schema/strings"/>
  ...
  <!-- Match codelist values. Must use iso19139.mcp-1.4 because some 
	     19139 codelists are extended in mcp - if the codelist exists in
			 iso19139.mcp-1.4 then use that otherwise use iso19139 codelists 
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="30000" match="*[*/@codeList and $schema='iso19139.mcp-1.4']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.mcp-1.4codelists" required="no"/>

    <xsl:variable name="xpath" select="gn-fn-metadata:getXPath(.)"/>
    <xsl:variable name="isoType" select="if (../@gco:isoType) then ../@gco:isoType else ''"/>
    <xsl:variable name="elementName" select="name()"/>

		<!-- check iso19139.mcp-1.4 first, then fall back to iso19139 -->
    ...
    <xsl:call-template name="render-element">
 	...
 	<!--
    Take care of enumerations. Same as for codelists, check iso19139.mcp-1.4
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="30000" match="*[gn:element/gn:text and $schema='iso19139.mcp-1.4']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.mcp-1.4codelists" required="no"/>
