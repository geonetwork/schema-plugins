...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:gco="http://www.isotc211.org/2005/gco"
	xmlns:geonet="http://www.fao.org/geonetwork/iso19115-1-2013"
	...
	<!-- A set of templates use to convert thesaurus concept to 
       ISO19115-1-2013 fragments. -->
	
  <!-- uses functions from schema iso19139/process/process-utility.xsl -->
	<xsl:include href="../process/process-utility.xsl"/>
	
	<!-- Convert a concept to an ISO19115-1-2013 fragment with an Anchor 
        for each keywords pointing to the concept URI-->
	<xsl:template name="to-iso19115-1-2013-keyword-with-anchor">
		<xsl:call-template name="to-iso19115-1-2013-keyword">
			<xsl:with-param name="withAnchor" select="true()"/>
		</xsl:call-template>
	</xsl:template>
	
	
	<!-- Convert a concept to an ISO19115-1-2013 mri:MD_Keywords with an 
       XLink which will be resolved by XLink resolver. -->
	<xsl:template name="to-iso19115-1-2013-keyword-as-xlink">
		<xsl:call-template name="to-iso19115-1-2013-keyword">
	...
	<!-- Convert a concept to an ISO19115-1-2013 keywords.
    If no keyword is provided, only thesaurus section is adaded.
    -->
	<xsl:template name="to-iso19115-1-2013-keyword">
...
-->                     </xsl:otherwise>
	...
	<!-- Convert a concept to an ISO19115-1-2013 extent -->
	<xsl:template name="to-iso19115-1-2013-extent">
		...
		<xsl:for-each select="//keyword[thesaurus/key = $currentThesaurus]">
			<xsl:choose>
				<xsl:when test="$isService">
					<srv:extent>
						<xsl:copy-of select="iso19115-1-2013:make-iso-extent(geo/west, geo/south, geo/east, geo/north, value)"/>
					</srv:extent>
				</xsl:when>
				<xsl:otherwise>
					<mri:extent>
