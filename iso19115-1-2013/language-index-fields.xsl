...
<xsl:stylesheet version="1.0" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:variable name="fileIdentifier" select="/mds:MD_Metadata|*[contains(@gco:isoType,'mds:MD_Metadata')]/mds:metadataIdentifier/mcc:MD_Identifier[mcc:codeSpace/*='urn:uuid']/mcc:code/*"/>
    ...
    <xsl:template match="/">
        ...
        <Documents>
            <xsl:for-each select="/*[name(.)='mds:MD_Metadata' or contains(@gco:isoType,'MD_Metadata')]/mds:defaultLocale/lan:PT_Locale">
		...
		<xsl:template name="document">
			...
			<Document locale="{$isoLangId}">
				...
				<xsl:variable name="title"
					select="/*[name(.)='mds:MD_Metadata' or contains(@gco:isoType,'MD_Metadata')]/mds:identificationInfo//mri:citation//cit:title//lan:LocalisedCharacterString[@locale=$poundLangId]"/>
				...
				<xsl:apply-templates select="/*[name(.)='mds:MD_Metadata' or contains(@gco:isoType,'MD_Metadata')]" mode="metadata">
					<xsl:with-param name="langId" select="$poundLangId"/>
				</xsl:apply-templates>

				<xsl:apply-templates mode="index" select="*[name(.)='mds:MD_Metadata' or contains(@gco:isoType,'MD_Metadata')]">
					<xsl:with-param name="langId" select="$poundLangId"/>
				</xsl:apply-templates>
			</Document>
	</xsl:template>
	...
	<xsl:template match="*" mode="metadata">
      ...
      <!-- FIXME: I've never seen individualFirstName etc? -->
		...
		<!-- === Service stuff ===  -->
		<!-- Service type           -->
		<xsl:for-each select="mds:identificationInfo/srv:SV_ServiceIdentification/srv:serviceType/gco:LocalName|
			mds:identificationInfo/*[contains(@gco:isoType, 'SV_ServiceIdentification')]/srv:serviceType/gco:LocalName">
			<Field name="serviceType" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>

		<!-- Service version        -->
		<xsl:for-each select="mds:identificationInfo/srv:SV_ServiceIdentification/srv:serviceTypeVersion/gco:CharacterString|
			mds:identificationInfo/*[contains(@gco:isoType, 'SV_ServiceIdentification')]/srv:serviceTypeVersion/gco:CharacterString">
			<Field name="serviceTypeVersion" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
