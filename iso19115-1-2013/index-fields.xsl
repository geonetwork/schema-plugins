...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:variable name="fileIdentifier" select="/mds:MD_Metadata|*[contains(@gco:isoType,'mds:MD_Metadata')]/mds:metadataIdentifier/mcc:MD_Identifier[mcc:codeSpace/*='urn:uuid']/mcc:code/*"/>
	...
	<xsl:template match="/">
		...
		<Document locale="{$isoLangId}">
			...
			<xsl:apply-templates select="*[name(.)='mds:MD_Metadata' or @gco:isoType='mds:MD_Metadata']" mode="metadata"/>
			
			<xsl:apply-templates mode="index" select="*[name(.)='mds:MD_Metadata' or @gco:isoType='mds:MD_Metadata']"/>
	...
	<xsl:template match="*" mode="metadata">
		...
		<xsl:for-each select="mds:identificationInfo//mri:MD_DataIdentification|
			...
			<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
            <xsl:variable name="email" select="/mds:MD_Metadata/mds:contact[1]/cit:CI_Responsibility[1]/cit:contactInfo[1]/cit:CI_Contact[1]/cit:address[1]/cit:CI_Address[1]/cit:electronicMailAddress[1]/gco:CharacterString[1]"/>
      ...
      <!-- FIXME: BrowseGraphic needs improvement - there are changes
		...
		<!-- === Lineage  === -->
		<xsl:for-each select="mds:resourceLineage/*/mrl:source[@uuidref]">
			<Field  name="hassource" string="{string(@uuidref)}" store="false" index="true"/>
		</xsl:for-each>
		
		<xsl:for-each select="mds:dataQualityInfo/*/dqm:report/*/dqm:result">
			<xsl:if test="$inspire='true'">
				<!-- 
		...
		<xsl:for-each select="mds:dataQualityInfo/*/dqm:lineage/*/dqm:statement/gco:CharacterString">
		...
		<!-- Metadata type  -->
		<xsl:choose>
			<xsl:when test="mds:metadataScope">
		...
		<!-- Metadata on maps -->
		<xsl:variable name="isDataset" select="count(mds:hierarchyLevel[mcc:MD_ScopeCode/@codeListValue='dataset']) > 0"/>
		...
		<xsl:variable name="isStatic" select="count(mds:distributionInfo/mrd:MD_Distribution/
		...
		<xsl:variable name="isInteractive" select="count(mds:distributionInfo/mrd:MD_Distribution/
		...
		<xsl:variable name="isPublishedWithWMCProtocol" select="count(mds:distributionInfo/mrd:MD_Distribution/
		...
		<xsl:choose>
			...
			<xsl:when test="mds:identificationInfo/srv:SV_ServiceIdentification">
		...
		<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->		
		<xsl:for-each select="mds:metadataScope/mds:MD_MetadataScope/mds:resourceScope/mcc:MD_ScopeCode">
			<Field name="levelName" string="{string(@codeListValue)}" store="false" index="true"/>
		</xsl:for-each>

		<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->		

		<xsl:for-each select="mds:language/gco:CharacterString
			|mds:language/lan:LanguageCode/@codeListValue
			|mds:locale/lan:PT_Locale/lan:language/lan:LanguageCode/@codeListValue">
			<Field name="language" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>

		<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->		
    <!-- fileIdentifier is deprecated in favour of metadataIdentifier -->
		<xsl:for-each select="mds:metadataIdentifier/mcc:MD_Identifier">
    ...
    <!-- parentIdentifier is deprecated in favour of parentMetadata -->
		<xsl:for-each select="mds:parentMetadata/cit:CI_Citation/cit:identifier/mcc:MD_Identifier/mcc:code/gco:CharacterString">
			<Field name="parentUuid" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
		
		<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
		
    <!-- mds:dateInfo is change date -->
    ...
    <!-- FIXME: MD_BrowseGraphic for organisation logo should use mcc:linkage 
		...
		<xsl:for-each select="mds:contact/cit:CI_Responsibility/cit:party/cit:CI_Organisation">
