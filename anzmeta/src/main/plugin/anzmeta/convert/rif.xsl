...
<!-- stylesheet to convert anzmeta V2 in OAI-PMH ListRecords response to 
     RIF-CS via iso19139 -->
...
<xsl:template match="root">
		<!-- Export anzMeta v2 to iso19115/19139 XML -->
		<xsl:variable name="md">
			<xsl:copy-of select="anzmeta"/>
		</xsl:variable>

		<xsl:variable name="isomd">
			<xsl:apply-templates select="exslt:node-set($md)"/>
		</xsl:variable>

		<!-- Export the iso19115/19139 XML to RIF-CS -->
		<xsl:apply-templates select="$isomd/*[1]">
			<xsl:with-param name="origSource" select="/root/env/siteURL"/>
    	<xsl:with-param name="group" select="/root/env/siteName"/>
  		<xsl:with-param name="uuidRec" select="/root/env/uuid"/>
		</xsl:apply-templates>
</xsl:template>
...
<xsl:template match="gmd:MD_Metadata">
  ...
  <xsl:param name="uuidRec" select="/root/env/uuid"/>
	<xsl:element name="registryObjects">
		<xsl:attribute name="xsi:schemaLocation">
            <xsl:text>http://ands.org.au/standards/rif-cs/registryObjects http://services.ands.org.au/documentation/rifcs/schema/registryObjects.xsd</xsl:text>
		</xsl:attribute>
		<xsl:message>OrigSource is <xsl:value-of select="$origSource"/></xsl:message>
		<xsl:apply-templates select="." mode="collection">
			<xsl:with-param name="origSource" select="$origSource"/>
    	<xsl:with-param name="group" select="$group"/>
    	<xsl:with-param name="uuidRec" select="$uuidRec"/>
		</xsl:apply-templates>
	</xsl:element>
</xsl:template>
					...
					<xsl:element name="electronic">
						...
						<xsl:element name="value">
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select="concat($origSource,'/metadata.show?uuid=',$uuidRec)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:element>
					</xsl:element>
