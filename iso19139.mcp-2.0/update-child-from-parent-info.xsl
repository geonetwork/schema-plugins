...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="/root/update/parent/mcp:MD_Metadata"/>
    </xsl:template>

    <!-- ================================================================= -->

    <xsl:template match="/root/update/parent/mcp:MD_Metadata">
        <xsl:copy>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:fileIdentifier"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:language|gmd:characterSet"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:parentIdentifier"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:hierarchyLevel"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:hierarchyLevelName"/>
            ...
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:dataSetURI"/>
            ...
            <gmd:identificationInfo>
                <xsl:for-each select="/root/child/mcp:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification|
	            /root/child/mcp:MD_Metadata/gmd:identificationInfo/*[@gco:isoType='gmd:MD_DataIdentification']|
	            /root/child/mcp:MD_Metadata/gmd:identificationInfo/srv:SV_ServiceIdentification|
        		/root/child/mcp:MD_Metadata/gmd:identificationInfo/*[@gco:isoType='srv:SV_ServiceIdentification']">
	            	<xsl:copy>
	            		<xsl:copy-of select="@*"/>
	                    ...
	                    <xsl:call-template name="process">
	                        ...
	                        <xsl:with-param name="name" select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:pointOfContact"/>
	                        <xsl:with-param name="mode" select="$updateMode"/>
	                        <xsl:with-param name="subLevel" select="true()"/>
	                    </xsl:call-template>
	                    
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:resourceMaintenance"/>
	                    <xsl:copy-of select="gmd:graphicOverview"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:resourceFormat"/>
	                    
	                    <xsl:call-template name="process">
	                        <xsl:with-param name="update" select="$gmd-descriptiveKeywords"/>
	                        <xsl:with-param name="name" select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords"/>
	                        <xsl:with-param name="mode" select="$updateMode"/>
	                        <xsl:with-param name="subLevel" select="true()"/>
	                    </xsl:call-template>
	                    
	                    <!-- FIXME / TO BE DISCUSS following sections are replaced (excepts AggregationInfo preserved). -->
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:resourceSpecificUsage"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:resourceConstraints"/>
	                    <xsl:copy-of select="gmd:aggregationInfo"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:spatialRepresentationType"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:spatialResolution"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:langage"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:characterSet"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:topicCategory"/>
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:environmentDescription"/>
	                    
	                    <xsl:call-template name="process">
	                        <xsl:with-param name="update" select="$gmd-extent"/>
	                        <xsl:with-param name="name" select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:extent"/>
	                        <xsl:with-param name="mode" select="$updateMode"/>
	                        <xsl:with-param name="subLevel" select="true()"/>
	                    </xsl:call-template>
	                    
	                    <!-- FIXME / TO BE DISCUSS following sections are replaced/preserved  -->
	                    <xsl:copy-of select="/root/update/parent/mcp:MD_Metadata/gmd:identificationInfo/*/gmd:supplementalInformation"/>
		            	<xsl:copy-of select="srv:*"/>
		            
									<!-- copy any mcp elements at the end of the 
									     identificationInfo/* -->
			            <xsl:copy-of select="mcp:*"/>
	            	</xsl:copy>
	            </xsl:for-each>
	        </gmd:identificationInfo>
            ...
            <!-- FIXME / TO BE DISCUSS following sections are preserved -->
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:series"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:describes"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:propertyType"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:featureType"/>
            <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:featureAttribute"/>
						<!-- copy any mcp elements at the end of the mcp:MD_Metadata -->
			      <xsl:copy-of select="mcp:*"/>
        ...
        <xsl:variable name="childElement">
            <xsl:choose>
                <xsl:when test="$subLevel=true()">
                    <xsl:copy-of select="/root/child/mcp:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/*[name(.)=name($name)]|
                    /root/child/mcp:MD_Metadata/gmd:identificationInfo/*[@gco:isoType='gmd:MD_DataIdentification']/*[name(.)=name($name)]|
		            /root/child/mcp:MD_Metadata/gmd:identificationInfo/srv:SV_ServiceIdentification/*[name(.)=name($name)]|
    	    		/root/child/mcp:MD_Metadata/gmd:identificationInfo/*[@gco:isoType='srv:SV_ServiceIdentification']/*[name(.)=name($name)]"/>        
                </xsl:when>
                <xsl:otherwise>
                    <xsl:copy-of select="/root/child/mcp:MD_Metadata/*[name(.)=name($name)]"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
