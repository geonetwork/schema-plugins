	...
	<xsl:template match="mcp:MD_Metadata">
		<oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/"
			...
			<xsl:for-each select="gmd:identificationInfo/mcp:MD_DataIdentification">
				...
				<xsl:for-each select="gmd:resourceConstraints/gmd:MD_DataCommons">
					<dc:rights><xsl:value-of select="gmd:licenseLink/gmd:URL"/></dc:rights>
				</xsl:for-each>

				<xsl:for-each select="gmd:resourceConstraints/gmd:MD_CreativeCommons">
					<dc:rights><xsl:value-of select="gmd:licenseLink/gmd:URL"/></dc:rights>
				</xsl:for-each>
