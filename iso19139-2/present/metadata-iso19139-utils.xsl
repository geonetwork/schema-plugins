...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="getCodeListValue">
    ...
    <xsl:variable name="codelist">
      <xsl:call-template name="getCodeListValues">
        <xsl:with-param name="qname" select="$qname"/>
        <xsl:with-param name="schema" select="$schema"/>
      </xsl:call-template>
    </xsl:variable>
    
    <xsl:value-of select="$codelist/entry[code = $value]"/>
  </xsl:template>
  
  <!-- Return all codelist values -->
  <xsl:template name="getCodeListValues">
    <xsl:param name="schema"/>
    <xsl:param name="qname"/>
	...
	<xsl:template name="relatedResources">
		...
		<xsl:variable name="metadata" select="/root/gmi:MI_Metadata|/root/*[@gco:isoType='gmd:MD_Metadata']"/>
