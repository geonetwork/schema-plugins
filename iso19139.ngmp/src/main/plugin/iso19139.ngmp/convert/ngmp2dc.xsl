	...
	<xsl:template match="gmd:MD_Metadata">
		<oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/"
			...
			<xsl:for-each select="gmd:identificationInfo/gmd:MD_DataIdentification">
                ...
                <xsl:for-each select="gmd:descriptiveKeywords/gmd:MD_Keywords/gmd:keyword/ngmp:*">
					<dc:subject><xsl:value-of select="local-name(.)"/>:<xsl:value-of select="@codeListValue"/></dc:subject>
				</xsl:for-each>
